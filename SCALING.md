...
| Rate limit counters (`express-rate-limit` default memory store) | Each replica allows the full quota, so an abuser across N replicas gets N× the limits. | Use `rate-limit-redis`, or pin the limiter to a single ingress. |
